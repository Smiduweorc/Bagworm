#!/bin/sh
# Bagworm installer: downloads the latest release binary for this platform.
#
#   curl -sSfL https://raw.githubusercontent.com/Smiduweorc/bagworm/master/install.sh | sh
#
# Environment:
#   BAGWORM_VERSION  release tag to install (default: latest)
#   BINDIR           install directory (default: /usr/local/bin)
set -eu

REPO="Smiduweorc/bagworm"
BINDIR="${BINDIR:-/usr/local/bin}"
VERSION="${BAGWORM_VERSION:-latest}"
...
num="${VERSION#v}"
tarball="bagworm_${num}_${os}_${arch}.tar.gz"
...
if [ -w "$BINDIR" ]; then
  install -m 0755 "$tmp/bagworm" "$BINDIR/bagworm"
else
  sudo install -m 0755 "$tmp/bagworm" "$BINDIR/bagworm"
fi

echo "Installed bagworm ${VERSION} to ${BINDIR}/bagworm"
